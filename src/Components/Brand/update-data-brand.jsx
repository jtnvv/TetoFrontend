...
export default function UpdateDataBrand({
  onClose,
  name,
  email,
  description,
  ...
  const handleSubmit = async (event) => {
    ...
    await UpdateBrandInformation(data)
      .then((res) => {
        if (email !== emailState) {
          // Si el correo electrónico se actualizó, cierra la sesión
      ...
      .catch((err) => {
        alert(
          "NO se ha actualizado la información: " + err.response.data.message
        );
      });
  };
  ...
  return (
    <div className="fixed  z-50 inset-0 bg-gray-900 bg-opacity-60 overflow-y-auto h-full w-full px-4  ">
      <div className="relative top-20 mx-auto shadow-xl rounded-md bg-white max-w-md font-default  ">
        <button
          onClick={onClose}
          className="absolute right-0 top-0 m-2 bg-transparent "
        >
          X
        </button>
          ...
          <form className="space-y-5">
            ...
            <div>
              ...
              <p className="text-sm text-red-500 mt-3">
                * Al modificar este campo, se cerrará la sesión
              </p>
            </div>
            ...
            <div>
              ...
              <input
                ...
                className="mt-4 font-default text-sm items-center text-black mb-3"
                id="imageUrl"
                name="imageUrl"
                type="file"
              />
              {imageStateShow && (
                <img
                  src={imageStateShow}
                  alt="brand"
                  className="h-20 w-full  md:h-full md:w-20 "
                />
              )}
            </div>
